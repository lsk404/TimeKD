...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path="/data", flag='train', size=None, 
        ...
        if not data_path.endswith('.csv'):
            ...
            data_path_dir = data_path[:-4]
            data_path_file = data_path
        self.data_path_dir = data_path_dir
        self.data_path_file = data_path_file
        self.model_name = model_name
        self.embed_path = f"{data_path_dir}/{self.pred_len}/{flag}/"

        self.__read_data__()

    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(self.data_path_file)
...
class Dataset_ETT_minute(Dataset):
    def __init__(self, root_path="/data", flag='train', size=None, 
        ...
        data_path = os.path.join(root_path, data_path)
        if not data_path.endswith('.csv'):
            data_path_dir = data_path # 文件路径（不含有csv）
            data_path_file = data_path+'.csv' # 以.csv结尾的文件路径
        else:
            data_path_dir = data_path[:-4]
            data_path_file = data_path
        self.data_path_file = data_path_file
